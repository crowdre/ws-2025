...
//var originalTime = moment(document.querySelector("#session-time-local").textContent + " September 21 2021", "HH:mm MMMM D YYYY");
const originalTime = Array.from(document.querySelectorAll("#session-time-local")).map(x => moment(x.textContent+" September 21 2021", "HH:mm MMMM D YYYY"));
...
$(".js-Selector").on("change", e => {
  ...
  {
    const timestamp = originalTime[i].unix();
    const offset = moment.tz(e.target.value).utcOffset() * 60;
    const dateTimeLocal = moment.unix(timestamp + offset);
    if (dateTimeLocal.format("D") != "21")
    {
        inputTime[i].textContent = dateTimeLocal.format("HH:mm") + " (" + dateTimeLocal.format("MMM D") + ")";
    }
    else
    {
        inputTime[i].textContent = dateTimeLocal.format("HH:mm");
    }
  }
...
console.log($('.js-Selector').length)
